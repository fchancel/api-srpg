...
class TestUser(unittest.TestCase):
    ...
    def test_create_user_with_invalid_token(self):
        ...
        self.assertEqual(response.json()['detail'], 'Invalid Token', msg=f"expected status code 401")
    ...
    def test_create_user_with_no_token_key(self):
        data = {}
        response = requests.post(url=self.url, json=data)
        self.assertEqual(response.status_code, 422, msg=f"expected status code 422")
    ...
    def test_create_correct_user(self):
        ...
        self.assertEqual(response.json()['token_srpg'], data['token_srpg'], msg='excepted same token_srpg between response and data')
