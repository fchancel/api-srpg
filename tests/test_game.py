...
from api.crud import get_character, get_mission_playing, get_user, create_village
...
class TestGame(unittest.TestCase):
    ...
    def test_create_game_bad_rank(self):
        ...
        response = requests.post(
            url=self.url+'/start', headers=self.header, json=data)
        self.assertEqual(response.status_code, 422,
                         msg='excepted status code 422')
    ...
    def test_create_game_success_and_create_game_already_exist(self):
        ...
        self.assertEqual(response.status_code, 201,
                         msg="excepted status code 201")
        self.assertEqual(response.json()[
                         "mission"], f"{get_settings().app_url}api/games/missions/2", msg="excepted same url")
        response = requests.post(
            url=self.url+'/start', headers=self.header, json=data)
        self.assertEqual(response.status_code, 409,
                         msg="excepted status code 409 | Mission already exist")

    def test_get_game_without_mission(self):
        response = requests.get(
            url=self.url+"/in-progress", headers=self.header)
        self.assertEqual(response.status_code, 404,
                         msg='excpeted status code 404')
    ...
    def test_get_game_time_left(self):
        ...
        requests.post(url=self.url+'/start', headers=self.header, json=data)
        response = requests.get(
            url=self.url+"/in-progress/time-left", headers=self.header)
        self.assertGreaterEqual(
            response.json()['time'], 10790, msg='expected time greather than 1790 seconds')
        self.assertLessEqual(
            response.json()['time'], 10810, msg='expected time less than 1790 seconds')
    ...
    def test_get_game_time_left_after_adding_time(self):
        ...
        response = requests.get(
            url=self.url+"/in-progress/time-left", headers=self.header)
        self.assertGreaterEqual(
            response.json()['time'], 14390, msg='expected time greather than 1790 seconds')
        self.assertLessEqual(
            response.json()['time'], 14410, msg='expected time less than 1790 seconds')
    ...
    def test_get_game_result_not_finish_time(self):
        ...
        requests.patch(url=self.url+"/in-progress/step?id_choice=" +
                       str(id_choice), headers=self.header)
        with Session(engine_test) as db:
            mission_playing: MissionPlaying = get_mission_playing(
                db, user=get_user(db, id=1))
